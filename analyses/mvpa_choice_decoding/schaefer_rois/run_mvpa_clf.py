...
def parse_args():
    p = argparse.ArgumentParser(
        description='Run MVPA binary choice decoding for an atlas ROI set (one subject).'
    )
    p.add_argument('--sub-id', type=str, required=True)
    p.add_argument('--y-col', type=str, default='choseAccept')
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help='Optional output tag; default = <atlas>_<y-col>_logreg.',
    )
...
def main():
    ...
    analysis_tag = args.analysis_tag or f'{args.atlas}_{args.y_col}_logreg'
