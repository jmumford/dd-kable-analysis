#!/usr/bin/env python3
"""
Beta Series Analysis Script

This script runs the beta series model for a single subject and run.
It fetches BOLD data, creates the design matrix, computes contrasts,
and saves effect size maps with BIDS-style naming conventions.

Usage:
    python run_beta_series.py <subject_id> <run_number>

Example:
    python run_beta_series.py dmp0005 1
"""
...
def run_beta_series_analysis(sub_id: str, run: int) -> None:
    """
    Run beta series analysis for a single subject and run.
    ...
    print(f'\n{"=" * 70}')
    print('Running Beta Series Analysis')
    ...
    try:
        behav_data, events_data, desmat = make_design_matrix(cfg, sub_id, run_str)
    ...
    saved_files = []
    for i, (contrast_name, contrast_def) in enumerate(contrasts.items(), 1):
        try:
            ...
            # Format: sub-<subid>_ses-<ses>_task-<task>_run-<run>_contrast-<contrast>_output-effectsize.nii.gz
            filename = (
                f'sub-{sub_id}_'
                f'ses-{ses}_'
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Run beta series analysis for a single subject and run',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python run_beta_series.py dmp0005 1
  python run_beta_series.py dmp0010 2
        """,
    )
    parser.add_argument(
    ...
    # Run the analysis
    run_beta_series_analysis(args.subject_id, args.run)
