#!/bin/bash
#SBATCH --job-name=beta_series
#SBATCH --output=/oak/stanford/groups/russpold/users/buckholtz/DD_Kable/scripts/dd-kable-analysis/logs/beta_series_%A_%a.out
#SBATCH --error=/oak/stanford/groups/russpold/users/buckholtz/DD_Kable/scripts/dd-kable-analysis/logs/beta_series_%A_%a.err
#SBATCH --time=4:00:00
#SBATCH --mem=16G
...
# Beta Series Analysis - SLURM Job Array Submission Script
...
SCRIPT_DIR="${PROJECT_ROOT}/analyses/beta_series_analysis"
...
# Run the beta series analysis using uv
cd ${SCRIPT_DIR}
uv --directory "$PROJECT_ROOT" run "${SCRIPT_DIR}/run_beta_series.py" "$SUB_ID" "$RUN"
