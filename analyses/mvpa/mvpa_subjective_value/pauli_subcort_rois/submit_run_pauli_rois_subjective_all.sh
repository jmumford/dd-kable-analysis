#!/bin/bash
#SBATCH --job-name=mvpa_pauli_rois_sv
#SBATCH --output=/oak/stanford/groups/russpold/users/buckholtz/DD_Kable/scripts/dd-kable-analysis/logs/mvpa_pauli_rois_sv_%A_%a.out
#SBATCH --error=/oak/stanford/groups/russpold/users/buckholtz/DD_Kable/scripts/dd-kable-analysis/logs/mvpa_pauli_rois_sv_%A_%a.err
#SBATCH --time=4:00:00
#SBATCH --mem=16G
...
# Run one array task per subject listed in SUB_LIST.
...
if [ ${EXIT_STATUS} -eq 0 ]; then
  echo "SUCCESS: MVPA (subjective value) completed for sub-${SUB_ID}"
else
  echo "FAILED: MVPA (subjective value) failed for sub-${SUB_ID} (exit code: ${EXIT_STATUS})"
fi
