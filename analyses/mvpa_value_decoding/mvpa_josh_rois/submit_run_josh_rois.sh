#!/bin/bash
#SBATCH --job-name=mvpa_josh_rois
#SBATCH --output=/oak/stanford/groups/russpold/users/buckholtz/DD_Kable/scripts/dd-kable-analysis/logs/mvpa_josh_rois_%A_%a.out
#SBATCH --error=/oak/stanford/groups/russpold/users/buckholtz/DD_Kable/scripts/dd-kable-analysis/logs/mvpa_josh_rois_%A_%a.err
#SBATCH --time=4:00:00
#SBATCH --mem=16G
...
SCRIPT_DIR="${PROJECT_ROOT}/analyses/mvpa_value_decoding/mvpa_josh_rois"
PY_SCRIPT="${SCRIPT_DIR}/run_josh_rois_subject.py"
...
# Run (add/remove --verbose as desired)
uv --directory "${PROJECT_ROOT}" run python "${PY_SCRIPT}" --sub-id "${SUB_ID}" --y-col amount --verbose
