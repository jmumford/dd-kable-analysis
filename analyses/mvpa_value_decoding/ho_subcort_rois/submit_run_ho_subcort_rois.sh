#!/bin/bash
#SBATCH --job-name=mvpa_harvard_oxford_subcort
#SBATCH --output=/oak/stanford/groups/russpold/users/buckholtz/DD_Kable/scripts/dd-kable-analysis/logs/mvpa_harvard_oxford_subcort_%A_%a.out
#SBATCH --error=/oak/stanford/groups/russpold/users/buckholtz/DD_Kable/scripts/dd-kable-analysis/logs/mvpa_harvard_oxford_subcort_%A_%a.err
#SBATCH --time=4:00:00
#SBATCH --mem=16G
...
# Run Harvard-Oxford subcortical
uv --directory "${PROJECT_ROOT}" run python "${PY_SCRIPT}" \
  --sub-id "${SUB_ID}" \
  --atlas harvard_oxford_subcort \
  --y-col amount \
  --verbose
